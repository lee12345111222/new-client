...
const AfterEventSurvey: FC<AfterEventSurveyProps> = ({
    ...
    useEffect(() => {
        if (
            ...
            textAnswer.length > 0 ||
            (restPostSurveyPage &&
                !postSurveys[restPostSurveyPage - 1].mustAnswer)
        )
    ...
    const handleAddResult = (
        ...
        if (sending.sendingSurvey.selected.length > 0 || isFinished)
            if (resultRef.current.length > 0) {
                ...
                const filteredResults = resultRef.current.filter(
                    r => r.sendingSurvey.surveyId !== surveyId,
                );

                if (filteredResults.length > 0)
                    resultRef.current = [...filteredResults, sending];
    ...
    const handleNext = (
        ...
        if (identifier === '_submit') {
            ...
            if (resultRef.current.length > 0) {
                const selected = resultRef.current.filter(
    ...
    const handlePrev = (
        ...
        // 找到前一題
        const [prevSelected] = resultRef.current.filter(
        ...
        // 加入本題更新後結果
        if (selectedOption.length > 0 || textAnswer.length > 0)
            handleAddResult(surveyId, category, type, false);
        // 刪除本題選中選項
        if (type === 'single' || type === 'multiple')
                            ...
                            {restPostSurveyPage === 0 ? (
                                <div className="after-event-startBox">
                                    <div className="after-event-title">
                                        {intl.formatMessage({
                                            id: 'main.PostSurveyStartMessage',
                                        })}
                                    </div>
                                    <Button
