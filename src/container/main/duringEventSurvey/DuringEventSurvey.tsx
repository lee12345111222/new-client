import React, { FC, useState, useEffect, Dispatch, SetStateAction, MutableRefObject, Fragment } from 'react'
...
const DuringEventSurvey: FC<DuringEventSurvey> = ({
     ...
     */
    useEffect(() => {
        if (duringEventSelectedOption.length > 0 || duringEventTextAnswer.length > 0) setNextDisabled(false)
    ...
    const handleOptionClick = (pkg: any, surveyId: string) => {
        if (pkg.type === 'multiple') {
            if (!duringEventBoxChecked.includes(pkg.option + surveyId)) {
                setDuringEventBoxChecked((state) => [...state, pkg.option + surveyId])
                setDuringEventSelectedOption((state) => [...state, pkg])
            } else {
                setDuringEventBoxChecked((state) => state.filter((s) => s !== pkg.option + surveyId))
                setDuringEventSelectedOption((state) => state.filter((s) => s.option !== pkg.option))
            }
        } else if (pkg.type === 'single') {
            if (!duringEventBoxChecked.includes(pkg.option + surveyId)) {
                setDuringEventBoxChecked((state) => state.filter((s) => !s.includes(surveyId)))
                setDuringEventBoxChecked((state) => [...state, pkg.option + surveyId])
    ...
    const handleAddResult = (surveyId: string, category: string, type: string, isFinished: boolean) => {
        ...
        if (type === 'single' || type === 'multiple') {
            sendingSurvey = { surveyId, category, selected: duringEventSelectedOption, correct }
            setDuringEventSelectedOption([])
        } else {
            sendingSurvey = { surveyId, category, selected: duringEventTextAnswer, correct }
            setDuringEventTextAnswer('')
        }
        ...
        if (duringEventResultRef.current.length > 0) {
            const filteredResults = duringEventResultRef.current.filter((r) => r.sendingSurvey.surveyId !== surveyId)

            if (filteredResults.length > 0) duringEventResultRef.current = [...filteredResults, sending]
    ...
    const handleNext = (
        ...
        if (identifier === '_submit') {
            ...
            if (duringEventResultRef.current.length > 0) {
                const selected = duringEventResultRef.current.filter((s) => s.sendingSurvey.surveyId === nextSurveyId)
                // 選擇題
                if (nextType === 'single' || nextType === 'multiple') {
                    if (selected.length > 0) {
                        setDuringEventSelectedOption(selected[0].sendingSurvey.selected as any[])

                        for (const op of selected[0].sendingSurvey.selected) {
                            if (nextSurveyId && typeof op === 'object') {
                                setDuringEventBoxChecked((state) => [...state, op.option + nextSurveyId])
                            }
                        }
                    ...
                    if (selected.length > 0) {
                        setDuringEventTextAnswer(selected[0].sendingSurvey.selected as string)
    ...
    const handlePrev = (prevSurveyId: string, prevType: string, surveyId: string, type: string, category: string) => {
        // 找到前一題
        const [prevSelected] = duringEventResultRef.current.filter((s) => s.sendingSurvey.surveyId === prevSurveyId)
        // 刪除本題本來結果
        duringEventResultRef.current = duringEventResultRef.current.filter((r) => r.sendingSurvey.surveyId !== surveyId)
        // 加入本題更新後結果
        if (duringEventSelectedOption.length > 0 || duringEventTextAnswer.length > 0)
            handleAddResult(surveyId, category, type, false)
        // 刪除本題選中選項
        if (type === 'single' || type === 'multiple')
            setDuringEventBoxChecked((state) => state.filter((s) => !s.includes(surveyId)))
        ...
        if (prevType === 'single' || prevType === 'multiple')
            setDuringEventSelectedOption(prevSelected.sendingSurvey.selected as any[])
        else setDuringEventTextAnswer(prevSelected.sendingSurvey.selected as string)
            ...
            <div className="during-event-survey-wrap scroll-Control">
                <div className="during-event-survey-unlock">
                    ...
                    {middleResult.map((rlt:any, i:number) => {
                        return (
                            <div key={rlt.surveyId} className="survey-process-container">
                                <h3 className="survey-question">{middleSurveys[i].subject}</h3>
                                            ...
                                            <div className="survey-process-text">
                                                ...
                                                <div className="survey-process-text-right">
                                                    <span className="survey-process-text-percent">
                                                        {toPercent(rlt.result[r], rlt.total)}%
                                                    </span>
                                                </div>
                                            </div>
                                            <Progress
                                                strokeColor={correct.includes(r) ? '#669DF6' : '#DADCE0'}
                                                percent={toPercent(rlt.result[r], rlt.total)}
                                                showInfo={false}
                                            />
                                        </Fragment>
                                    )
        ...
        <div className="during-event-survey-wrap scroll-Control">
            <>
                {middleSurveyShow && userMiddleSurveys ? (
                    ...
                    <>
                        {middleSurveyShow && middleSurveys && middleSurveys.length > 0 ? (
                            <>
                                {restMiddleSurveyPage === 0 ? (
                                    <div className="after-event-startBox">
                                        <div className="after-event-title">
                                            {intl.formatMessage({ id: 'main.QuizStartMessage' })}
                                        </div>
                                        <Button className="during-event-survey-prev-btn" onClick={handleSurveyStart}>
                                    ...
                                    <>
                                        <div className="during-event-survey-QuestionBox">
                                            <div className="after-event-Question">
                                                {(middleSurveys as Survey[])[restMiddleSurveyPage - 1].subject}
                                            </div>
                                            <div className="during-event-survey-body">
                                                {(middleSurveys as Survey[])[restMiddleSurveyPage - 1].type ===
                                                    'single' ||
                                                (middleSurveys as Survey[])[restMiddleSurveyPage - 1].type ===
                                                    'multiple' ? (
                                                    ...
                                                    <div className="during-event-survey-textarea-box">
                                                        <TextArea
                                                            ...
                                                            value={duringEventTextAnswer}
                                                            onChange={(e) => setDuringEventTextAnswer(e.target.value)}
                                                ...
                                                <div className="during-event-survey-btn-group">
                                                    {restMiddleSurveyPage - 1 <
                                                        (middleSurveys as Survey[]).length - 1 && (
                                                        <Button
                                                            ...
                                                            onClick={() =>
                                                                handleNext(
                                                                    ...
                                                                    '_next',
                                                                    (middleSurveys as Survey[])[restMiddleSurveyPage]
                                                                        .surveyId,
                                                                    (middleSurveys as Survey[])[restMiddleSurveyPage]
                                                                        .type
                                                                )
                                                    ...
                                                    {restMiddleSurveyPage - 1 > 0 && (
                                                        <Button
                                                            className="during-event-survey-prev-btn"
                                                            onClick={() =>
                                                                handlePrev(
                                                                    (middleSurveys as Survey[])[
                                                                        restMiddleSurveyPage - 2
                                                                    ].surveyId,
                                                                    (middleSurveys as Survey[])[
                                                                        restMiddleSurveyPage - 2
                                                                    ].type,
                            ...
                            <div className="during-event-survey-before-start">
                                <div className="during-event-survey-before-start-text">
                                    {intl.formatMessage({ id: 'main.QuizBefore' })}
                                </div>
                            </div>
                        )}
                    </>
